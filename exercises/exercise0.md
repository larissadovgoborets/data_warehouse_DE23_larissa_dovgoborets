...
| modern data stack | collection of cloud-based tools and technologies used for data integration, storage, processing, and analysis, designed to streamline data workflows and enhance business intelligence. Typically includes components for data ingestion, data warehousing, data transformation, and analytics, leveraging scalability and flexibility of cloud infrastructure.            |
...
| OLAP              | Online Analytical Processing - technology that allows users to interactively analyze multidimensional data from multiple perspectives, facilitating complex queries and data analysis for business intelligence. Supports fast querying and reporting by pre-aggregating data into cubes or other structures.            |
| OLTP              | Online Transaction Processing is a database processing system optimized for handling a large number of short, real-time transactions, such as insertions, updates, and deletions. Focuses on operational tasks and maintaining data integrity in real-time, typically supporting day-to-day business operations.            |
...
| permanent table   | standard table that persists data indefinitely until explicitly deleted or altered. Designed for long-term storage and management of data, supporting features like indexing and access control.            |
...
| view              | virtual table that provides a predefined query result from one or more tables. Allows users to simplify complex queries, encapsulate logic, and present data in a specific format without storing the data physically.            |
| table             | fundamental database object that stores structured data in rows and columns. Used to organize and manage data for querying, analysis, and reporting, with defined schemas to ensure data consistency and integrity.            |
| DML               | Data Manipulation Language - SQL commands used to manage and manipulate data within a database, e.g. SELECT (retrieve data), INSERT (add new data), UPDATE (modify existing data), and DELETE (remove data).            |
| DDL               | Data Definition Language - SQL commands used to define and manage database structures, e.g. CREATE (establish new database objects), ALTER (modify existing objects), and DROP (remove objects).            |
| DQL               | Data Query Language - SQL commands used to retrieve and query data from a database, e.g. SELECT and its variations such SELECT INTO, SELECT DISTINCT, SELECT WHERE (fetch and view data based on specified criteria).            |
| DCL               | Data Control Language - SQL commands used to manage permissions and access control in a database, e.g. GRANT (to assign privileges) and REVOKE (to remove privileges)            |
